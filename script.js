...
// Top brand bar background change on scroll
window.addEventListener('scroll', function() {
    const topBrand = document.querySelector('.top-brand');
    if (topBrand) {
        if (window.scrollY > 100) {
            topBrand.style.background = 'rgba(255, 255, 255, 0.98)';
            topBrand.style.backdropFilter = 'blur(15px)';
        } else {
            topBrand.style.background = 'rgba(255, 255, 255, 0.95)';
            topBrand.style.backdropFilter = 'blur(10px)';
        }
    }
});
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Close side nav when clicking on a link (mobile)
    ...
    // Adjust body margin when side nav is hovered
    const sideNav = document.querySelector('.side-nav');
